...
if (searcherTab) {
    const titleValue = document.querySelector("h1[id='title']").getAttribute("data-title");
...
const mobileNav = document.querySelector('#top-navigation > ul > li[class="dropdown mobile-only"] > ul');

if (optionsDropdown) {
    optionsDropdown.insertAdjacentHTML('afterbegin', "<li><a href='/sub-etha' target='_blank'>sub-etha</a></li>");
}
if (mobileNav) {
    mobileNav.insertAdjacentHTML('afterbegin', "<li class='mobile-only'><a href='/sub-etha' target='_blank'>sub-etha</a></li>");
}

// Sayfa boyutu ve entry numaraları yönetimi
const optionsLink = document.querySelector("a[href='/ayarlar/tercihler']");
const topicPageSize = optionsLink ? sessionStorage.getItem("topicPageSize") : 10;

if (optionsLink && topicPageSize === null) {
    fetch(optionsLink.href)
        .then(response => response.text())
        .then(result => {
            const match = result.match(/<input\s+type="hidden"\s+id="TopicPageSize"\s+name="TopicPageSize"\s+value="([0-9]+)"\s*\/>/);
...
function setEntryNumbers(entryPerPage) {
    ...
    if (entryWrapper) {
        ...
        if (moreElement?.classList.contains('showall') && moreElement?.classList.contains('more-data')) {
        ...
        Array.from(entries).forEach((entry, index) => {
            ...
            if (permalink) {
                permalink.insertAdjacentText('afterbegin', `#${entry.getAttribute("data-id")} `);
            }
        });
    }
}
...
document.querySelectorAll('div.info a.entry-author').forEach(author => {
    author.insertAdjacentHTML('afterend', '<a class="icon icon-mail custom-message" title="mesaj at" aria-label="mesaj at"></a>');
});

document.addEventListener('click', (e) => {
    if (e.target.classList.contains('custom-message')) {
        const messageLink = e.target.nextElementSibling.querySelector('ul.dropdown-menu li a[title="mesaj at"]');
        messageLink?.click();
    }
});
